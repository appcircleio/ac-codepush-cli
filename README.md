...
After installing CodePush CLI globally, it will be available under `appcircle-code-push`.
...
```
appcircle-code-push app add <appName>
```
...
```
appcircle-code-push app add MyApp-Android
appcircle-code-push app add MyApp-iOS
```
...
```
appcircle-code-push app rename <appName> <newAppName>
```
...
```
appcircle-code-push app rm <appName>
```
...
```
appcircle-code-push deployment add <appName> <deploymentName>
```

If you want to re-use an existing deployment key, you can do this with:

```
appcircle-code-push deployment add <appName> <deploymentName> -k <existing-deployment-key>
```

Just like with apps, you can remove and rename deployments as well, using the following commands respectively:

```
appcircle-code-push deployment rm <appName> <deploymentName>
appcircle-code-push deployment rename <appName> <deploymentName> <newDeploymentName>
```
...
2. [React Native](#releasing-updates-react-native) - Performs the same functionality as the general release command, but also handles the task of generating the updated app contents for you (JS bundle and assets), instead of requiring you to run both `react-native bundle` and then `appcircle-code-push release`.
...
```
appcircle-code-push release <appName> <updateContents> <targetBinaryVersion>
...
This specifies the name of the CodePush app that this update is being released for. This value corresponds to the friendly name that you specified when originally calling `appcircle-code-push app add` (e.g. "MyApp-Android"). If you need to look it up, you can run the `appcircle-code-push app ls` command to see your list of apps.
...
```shell
appcircle-code-push release-react <appName> <platform>
...
appcircle-code-push release MyApp-iOS ./CodePush 1.0.0
```
...
```shell
appcircle-code-push release-react MyApp-iOS ios
```
...
appcircle-code-push release-react MyApp-Android android -p "./foo/bar/build.gradle"
```
...
```shell
appcircle-code-push release-react MyApp-iOS ios -p "./foo/bar/MyFile.plist"
```
...
appcircle-code-push release-react MyApp-iOS ios --pre "STAGING"

# Tell the CLI to use your dev plist (`DEV-Info.plist`).
# Note that the hyphen separator can be explicitly stated.
appcircle-code-push release-react MyApp-iOS ios --pre "DEV-"
```
...
```shell
appcircle-code-push patch <appName> <deploymentName>
[--label <releaseLabel>]
...
appcircle-code-push patch MyApp-iOS Production -m

# Increase the rollout for v23 to 50%
appcircle-code-push patch MyApp-iOS Production -l v23 -rollout 50%
```

### Label parameter

Indicates which release (e.g. `v23`) you want to update within the specified deployment. If ommitted, the requested changes will be applied to the latest release in the specified deployment. In order to look up the label for the release you want to update, you can run the `appcircle-code-push deployment history` command and refer to the `Label` column.
...
appcircle-code-push patch MyApp-iOS Staging -t "1.0.0 - 1.0.5"
```
...
```
appcircle-code-push promote <appName> <sourceDeploymentName> <destDeploymentName>
[--description <description>]
[--label <label>]
[--disabled <disabled>]
[--mandatory]
...
appcircle-code-push promote MyApp-iOS Staging Production -t "*"
```
...
```
appcircle-code-push rollback <appName> <deploymentName>
appcircle-code-push rollback MyApp-iOS Production
```
...
```
appcircle-code-push rollback MyApp-iOS Production --targetRelease v34
```
...
```
appcircle-code-push deployment history <appName> <deploymentName>
```
