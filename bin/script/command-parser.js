...
yargs
    ...
    yargs
        ...
        demand: false,
        description: "The base URL of the CodePush server you want to connect to. Example: https://api-my.server.appcircle.io",
        type: "string",
    })
        .option("authUrl", {
        alias: null,
        ...
        demand: false,
        description: "The authentication endpoint base URL used to obtain access tokens. Example: https://auth-my.server.appcircle.io",
        type: "string",
    })
        .check((argv, aliases) => isValidCommand); // Report unrecognized, non-hyphenated command category.
    addCommonConfiguration(yargs);
    ...
    if (!wasHelpShown && argv._ && argv._.length > 0) {
        ...
        switch (arg0) {
            ...
            case "login":
                ...
                loginCommand.serverUrl = getServerUrl(argv["serverUrl"]);
                loginCommand.authUrl = getServerUrl(argv["authUrl"]);
                loginCommand.pat = argv["accessKey"];
                break;
            case "logout":
