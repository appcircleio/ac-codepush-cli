    ...
    return Q(null).then(() => {
        switch (command.type) {
            ...
            default:
                ...
                if (!connectionInfo) {
                    throw new Error("You are not currently logged in. Run the 'appcircle-code-push login' command to authenticate with the CodePush server.");
                }
                exports.sdk = getSdk(connectionInfo.accessKey, null, CLI_HEADERS, connectionInfo.customServerUrl, connectionInfo.customAuthUrl);
                break;
        }
        switch (command.type) {
...
function login(command) {
    ...
    if (command.pat) {
        exports.sdk = getSdk(null, command.pat, CLI_HEADERS, command.serverUrl, command.authUrl);
        return exports.sdk.isAuthenticated().then((isAuthenticated) => {
            if (isAuthenticated) {
                serializeConnectionInfo(exports.sdk.accessKey, /*preserveAccessKeyOnLogout*/ true, command.serverUrl, command.authUrl);
            }
            else {
...
function loginWithExternalAuthentication(action, serverUrl, authUrl) {
...
function serializeConnectionInfo(accessKey, preserveAccessKeyOnLogout, customServerUrl, customAuthUrl) {
...
function getSdk(accessKey, pat, headers, customServerUrl, customAuthUrl) {
    const sdk = new AccountManager(accessKey, pat, CLI_HEADERS, customServerUrl, customAuthUrl);
    /*
